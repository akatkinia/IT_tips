...
# преобразуем json в питоновский объект (список)
people_list = json.loads(people)

# сортировка словаря по возрасту и имени
sorted_people = sorted(people_list, key=lambda x: (x['age'], x['name']))
# key=lambda x: (x['age'], x['name']): Это анонимная (лямбда) функция, которая используется как ключ для сортировки. Лямбда-функция принимает один аргумент x, который представляет собой элемент списка people_list (это словарь, представляющий информацию о человеке).
# (x['age'], x['name']): Внутри лямбда-функции создается кортеж, содержащий два элемента. Первый элемент - это возраст человека (x['age']), который будет использоваться как первый критерий для сортировки. Второй элемент - это имя человека (x['name']), который будет использоваться как второй критерий для сортировки.
# Итак, список people_list будет отсортирован сначала по возрасту (по возрастанию) и затем, в случае совпадения возрастов, по имени (в алфавитном порядке).
...
for person in sorted_people:
    ...
    print(name, country, age, sep=', ')


##### Ещё один вариант решения, но довольно извращённый и намешанный (list comprehension)
# [ print(*person.values(), sep=', ')
#   for person in sorted(__import__('json').loads(people),
#   key=lambda person: (person['age'], person['name'])) ]


    # ##### Тот же довольно извращённый и намешанный вариант решения, но хотя бы не в одну строку
    # for person in sorted(__import__('json').loads(people),
    #   key=lambda person: (person['age'], person['name'])): 
    #       print(*person.values(), sep=', ')
